...
class TestLoadFromDict:
    ...
    def test_priority_and_timeout_override(self) -> None:
        config = load_from_dict({"hooks": [_minimal_entry(priority=77, timeout_ms=3000)]})
        entry = config.hooks[0]
...
class TestBuildRegistry:
        ...
        config = load_from_dict(
            {
                "hooks": [
                    _minimal_entry(
                        "my-scope-filter", "pre_inject", "scope_filter_hook", priority=95
                    )
                ]
            }
        )
    ...
    def test_hook_type_override_on_builtin(self) -> None:
        ...
        config = load_from_dict(
            {"hooks": [_minimal_entry("audit-violation", "on_violation", "audit_hook")]}
        )
        registry = build_registry(config)
    ...
    def test_unknown_hook_type_raises(self) -> None:
        config = load_from_dict({"hooks": [_minimal_entry(hook_type="not_a_type")]})
        with pytest.raises(HookConfigError, match="Unknown hook type"):
            build_registry(config)

    def test_unknown_builtin_raises(self) -> None:
        config = load_from_dict({"hooks": [_minimal_entry(builtin="not_a_builtin")]})
...
class TestConfigToExecutorIntegration:
    def test_scope_filter_aborts_out_of_scope(self) -> None:
        ...
        config = load_from_dict(
            {"hooks": [_minimal_entry("sf", "pre_inject", "scope_filter_hook", priority=95)]}
        )
        registry = build_registry(config)
