...
function _page($$renderer) {
  head("mhazmq", $$renderer, ($$renderer2) => {
    $$renderer2.title(($$renderer3) => {
      $$renderer3.push(`<title>Core Concepts - VCP Documentation</title>`);
    });
    $$renderer2.push(`<meta name="description" content="Understand the fundamental concepts of VCP: context, constitutions, privacy filtering."/>`);
  });
  {
    let children = function($$renderer2) {
      $$renderer2.push(`<h2>The Problem VCP Solves</h2> <p>Modern AI systems need user context to provide personalized experiences. But traditional
			...
			The AI knows your context <em>shaped</em> the response, but not <em>what</em> that context was.</p> <h2>VCP Context Structure</h2> <p>Every VCP context has these layers:</p> <h3>1. Profile Identity</h3> <pre><code>{
  vcp_version: "1.0",
  profile_id: "user_001",  // Unique identifier
  created: "2026-01-15",
  updated: "2026-01-21"
}</code></pre> <h3>2. Constitution Reference</h3> <p>Points to a constitution that defines AI behavioral guidelines:</p> <pre><code>{
  constitution: {
...
}</code></pre> <h3>3. Public Profile</h3> <p>Information always shared with stakeholders:</p> <pre><code>{
  public_profile: {
    display_name: "Alex",
    goal: "learn_guitar",
    experience: "beginner",
...
}</code></pre> <h3>4. Portable Preferences</h3> <p>Settings that follow you across platforms:</p> <pre><code>{
...
}</code></pre> <h3>5. Constraint Flags</h3> <p>Boolean flags indicating active constraints:</p> <pre><code>{
  constraints: {
...
}</code></pre> <h3>6. Private Context</h3> <p>Sensitive information that influences AI but is <strong>never transmitted</strong>:</p> <pre><code>{
  private_context: {
...
}</code></pre> <h3>Sharing Policies</h3> <p>Define what each stakeholder type can see:</p> <pre><code>{
  sharing_policy: {
    "platform": {
...
}</code></pre> <h2>Personas</h2> <p>Personas define interaction styles. The same constitution can use different personas for
			different contexts:</p> <table><thead><tr><th>Persona</th><th>Style</th><th>Best For</th></tr></thead><tbody><tr><td><i class="fa-solid fa-palette" aria-hidden="true"></i> <strong>Muse</strong></td><td>Creative, exploratory, encouraging</td><td>Creative work, learning, exploration</td></tr><tr><td><i class="fa-solid fa-shield" aria-hidden="true"></i> <strong>Sentinel</strong></td><td>Cautious, protective, conservative</td><td>Security, safety-critical decisions</td></tr><tr><td><i class="fa-solid fa-people-group" aria-hidden="true"></i> <strong>Godparent</strong></td><td>Nurturing, supportive, patient</td><td>Education, skill building, recovery</td></tr><tr><td><i class="fa-solid fa-handshake" aria-hidden="true"></i> <strong>Ambassador</strong></td><td>Professional, diplomatic, balanced</td><td>Business, negotiations, formal contexts</td></tr><tr><td><i class="fa-solid fa-anchor" aria-hidden="true"></i> <strong>Anchor</strong></td><td>Stable, grounding, realistic</td><td>Crisis support, reality checking</td></tr><tr><td><i class="fa-solid fa-baby" aria-hidden="true"></i> <strong>Nanny</strong></td><td>Structured, directive, safe</td><td>Children, vulnerable users, strict guidance</td></tr></tbody></table> <h2>Audit Trails</h2> <p>VCP maintains cryptographically verifiable audit trails of all data sharing:</p> <pre><code>{
  audit_entry: {
    id: "aud_001",
...
}</code></pre> <h2>Next Steps</h2> <ul><li><a href="/docs/csm1-specification">CSM-1 Specification</a> — The token format in detail</li> <li><a href="/docs/api-reference">API Reference</a> — All VCP library functions</li> <li><a href="/demos">Interactive Demos</a> — See VCP in action</li></ul>`);
    };
    DocsLayout($$renderer, {
      title: "Core Concepts",
