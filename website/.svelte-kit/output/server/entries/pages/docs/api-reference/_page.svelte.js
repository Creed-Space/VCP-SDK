...
}</code></pre> <h3>PRIVATE_MARKER / SHARED_MARKER</h3> <pre><code>const PRIVATE_MARKER = '&lt;i class="fa-solid fa-lock" aria-hidden="true">&lt;/i>';
const SHARED_MARKER = '&lt;i class="fa-solid fa-check" aria-hidden="true">&lt;/i>';</code></pre> <h2>Enums</h2> <h3>PersonaType</h3> <pre><code>type PersonaType = 'muse' | 'ambassador' | 'godparent' | 'sentinel' | 'anchor' | 'nanny';</code></pre> <h3>ExperienceLevel</h3> <pre><code>type ExperienceLevel = 'beginner' | 'intermediate' | 'advanced' | 'expert';</code></pre> <h3>LearningStyle</h3> <pre><code>type LearningStyle = 'visual' | 'auditory' | 'hands_on' | 'reading' | 'mixed';</code></pre> <h3>NoiseMode</h3> <pre><code>type NoiseMode = 'normal' | 'quiet_preferred' | 'silent_required';</code></pre> <h3>SessionLength</h3> <pre><code>type SessionLength = '15_minutes' | '30_minutes' | '60_minutes' | 'flexible';</code></pre> <h3>BudgetRange</h3> <pre><code>type BudgetRange = 'unlimited' | 'high' | 'medium' | 'low' | 'free_only';</code></pre> <h3>ScopeType</h3> <pre><code>type ScopeType = 'work' | 'education' | 'creativity' | 'health' | 'privacy'
